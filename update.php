...
// data to be show in the input field 
if(isset($_GET['id'])){
    $id = $_GET['id'];
    $quer = "SELECT * FROM todo_table where id={$id}";
    ...
    //    data to be Update
       if(isset($_POST['update-data'])){
        $updateTodo = $_POST['todo'];
        $id = $_GET['id'];
        $time = $_POST['time'];
        $sql = "UPDATE todo_table SET id=$id, todo='$updateTodo', tm='$time' WHERE id={$id}";
        $query=mysqli_query($con,$sql);
...
<h1 class="text-center pb-3 w-100"> <img src="images/book.svg" height="30"></span> My ToDo List <img src="images/pencil.svg" height="30"></span>  </h1>
  <form action="" method="post" class="mx-auto" style="">
  <div class="input-group mb-3 mx-auto">

  <input type="text" id="input" class=" inp form-control  py-2"  placeholder="Your today's ToDo list" aria-label="Your today's ToDo list" value="<?php echo $data['todo'];?>"name="todo" required>
  <div class="input-group-append bb">
    <input type="time" placeholder="Time" class="input-group-text px-4  text-white font-weight-bolder bg-success border-secondary" value="<?php echo $data['tm'];?>" id="basic-addon2" name="time">
  </div>
  <div class="input-group-append bb">
    <button class="input-group-text px-4 bb text-white font-weight-bolder bg-success border-secondary" id="basic-addon2" name="add"> Add <span class="ml-3">
 <img src="images/add.svg" height="30"></span> </button>
  </div>
</div>

  </div>
  <center>
  <button class="btn btn-warning  mb-5 text-white font-weight-bolder " id="basic-addon2" name="update-data" style="border-radius:50px;box-shadow:0px 0px 10px black;font-size:20px;"> Update <span class=""><img src="images/browser.svg" height="40"></span> </button></center>
  </form>
